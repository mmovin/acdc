    ...
    **kws,
    ...
    if cache:
        cache_path = os.path.join(
            get_models_home(models_home), dataset, str(model_number), os.path.basename(full_path)
        )
...
def load_trained_model(
    ...
    # save location
    cache_path = os.path.join(
        get_models_home(models_home), data_name, str(model_number), f"{save_name}.{ext}"
    )
...
def save_model(
    ...
    Parameters
    ----------
    model_number: For training multiple models.

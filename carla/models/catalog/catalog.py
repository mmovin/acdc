    ...
    def __init__(
        ...
        **kws
        ...
        if isinstance(data, OnlineCatalog):
            # Load catalog
            catalog_content = ["ann", "linear", "forest"]
        ...
        if load_online:
            self._model = load_online_model(
                model_type, data.name, ext, cache, models_home, model_number=self._model_number, **kws
            )


    def __call__(self, x: Union[np.ndarray, pd.DataFrame, torch.Tensor, tf.Tensor]

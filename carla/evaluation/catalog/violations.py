...
## Added by Movin et al. to evaluate violation of plausibility constraint for BC dataset.
def constraint_bc(counterfactuals: pd.DataFrame):
    num = []
    for _, cf in counterfactuals.iterrows():
        valid = True
        cf_np = cf.to_numpy()
        for i in range(10):
            if cf_np[i] > cf_np[i+20]:
                valid = False
        if valid:
            num.append(0)
        else:
...
def constraint_violation(
    ...
    ## For BC dataset check feasibility instead:
    if data.name == 'breast_cancer':
        return constraint_bc(df_decoded_cfs)
